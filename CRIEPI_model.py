...
class AirconModel_CRIEPI(AirconSpec):
    """
    ...
    def _calc_cooling_efficiency(self, Q, P, V_inner, V_outer):
        ...
        冷房時の熱効率を計算。
        """
        M_evp = (1 - BF) * ae.air_density(aeJIS.T_C_IN) * V_inner
        T_evp = aeJIS.T_C_IN - Q / (M_evp * ae.air_specific_heat(aeJIS.X_C_IN))
        T_evp = avoid_over_saturation(T_evp, aeJIS.X_C_IN)
        M_cnd = (1 - BF) * ae.air_density(aeJIS.T_C_EX) * V_outer
        T_cnd = aeJIS.T_C_EX + (Q + P) / (M_cnd * ae.air_specific_heat(aeJIS.X_C_EX))
    ...
    def _calc_heating_efficiency(self, Q, P, V_inner, V_outer):
        ...
        暖房時の熱効率を計算。
        """
        M_evp = (1 - BF) * ae.air_density(aeJIS.T_H_EX) * V_outer
        T_evp = aeJIS.T_H_EX - (Q - P) / (M_evp * ae.air_specific_heat(aeJIS.X_H_EX))
        T_evp = avoid_over_saturation(T_evp, aeJIS.X_H_EX)
        M_cnd = (1 - BF) * ae.air_density(aeJIS.T_H_IN) * V_inner
        T_cnd = aeJIS.T_H_IN + Q / (M_cnd * ae.air_specific_heat(aeJIS.X_H_IN))
